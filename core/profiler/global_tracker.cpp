...
bool GlobalTracker::detect_cycle(const pid_t tid, pthread_mutex_t* mid)
{
...
bool GlobalTracker::dfs(void* node, std::unordered_map<void*, bool>& visited,
                        std::unordered_map<void*, bool>& rec_stack)
{
  if (!visited[node])
  {
...
void GlobalTracker::remove_edge(void* from, void* to)
{
  auto& vec = adj_[from];
